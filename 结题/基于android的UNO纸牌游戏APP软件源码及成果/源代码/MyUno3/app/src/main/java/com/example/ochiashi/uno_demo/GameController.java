	...
	public void drawFour() {
		...
		if (currentDirection == anticlockwise){
			for (int i = 0; i < 4; i++)
				alp.get((currentPlayer + 1) % playerNumber).drawCard(cg.draw());
			Common.rePosition(unoView, alp.get((currentPlayer + 1) % playerNumber), (currentPlayer + 1) % playerNumber);
			System.out.println("玩家手牌" + alp.get((currentPlayer + 1) % playerNumber).cardNumber() + "张");
			currentPlayer = (currentPlayer + 2) % playerNumber;
			...
			for (int i = 0; i < 4; i++)
				alp.get((currentPlayer + 3) % playerNumber).drawCard(cg.draw());
			Common.rePosition(unoView, alp.get((currentPlayer + 3) % playerNumber), (currentPlayer + 3) % playerNumber);
			System.out.println("玩家手牌" + alp.get((currentPlayer + 3) % playerNumber).cardNumber() + "张");
			currentPlayer = (currentPlayer + 2) % playerNumber;
...
//			drawFour();
	...
	public int nextPlayer(){
		...
		if (currentDirection == anticlockwise)
			...
			return (currentPlayer + 7) % playerNumber;
	}

	/*
	对家是谁
	 */
	public int facePlayer(){
		int result;
		if (currentDirection == anticlockwise){
			result = (currentPlayer + 1) % playerNumber;
			result = (result + 1) % playerNumber;
		} else{
			result = (currentPlayer + 7) % playerNumber;
			result = (result + 7) % playerNumber;
		}
		return result;
	}
